...
const ResetPassword = () => {
    ...
    const handleReset = async (e) => {
        e.preventDefault();
        toast.dismiss(); // Clear old errors to prevent stacking

        // 1. Length Validation
        if (password.length < 6) {
            return toast.warn("Password must be at least 6 characters long.", { position: "top-center" });
        }
        ...
        try {
            // ✅ Updated: Using backend_url variable
            const { data } = await axios.put(`${backend_url}/api/users/reset-password/${token}`, { password });
            
            if (data.success) {
                toast.success("Password updated! Redirecting to login...", { position: "top-center" });
                
                // Give user 3 seconds to see the success message
                setTimeout(() => {
            ...
            if (!error.response) {
                ...
                toast.error(
                    <span>Invalid or expired token. <br /> Please request a new link.</span>, 
                    { position: "top-center" }
                );
            } else {
